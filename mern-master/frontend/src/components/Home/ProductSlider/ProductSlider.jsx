...
const ProductSlider = ({ title, tagline }) => {
    ...
    return (
        <section className="bg-white w-full shadow overflow-hidden ">
            {/* <!-- header --> */}
            <div className="flex px-6 py-4 justify-between items-center border border-gray-300 shadow-md bg-gray-100">
                <div className="title flex flex-col gap-0.5 ">
                    <h1 className="text-xl font-medium">{title}</h1>
                    <p className="text-sm text-gray-400">{tagline}</p>
                </div>
                <Link to="/products" className="bg-pink-600 text-xs font-medium text-white px-5 py-2.5 rounded-full shadow uppercase hover:shadow-lg">view all</Link>
            </div>
            <hr />
            {loading ? null :
