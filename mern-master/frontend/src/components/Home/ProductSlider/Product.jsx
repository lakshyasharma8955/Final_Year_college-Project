    ...
    return (
        <div className="flex flex-col items-center gap-2 px-2 py-6 relative">
            ...
            <Link to={`/product/${_id}`} className="flex flex-col items-center text-center group">
                ...
                <h2 className="text-sm mt-4 group-hover:text-primary-blue font-serif">{name.length > 50 ? `${name.substring(0, 50)}...` : name}</h2>
            </Link>
            {/* <!-- image & product title --> */}
            ...
            <div className="flex flex-col gap-2 items-center">
                ...
                <span className="text-sm text-gray-500 font-medium flex gap-2 items-center">
                    <span className="text-xs px-1.5 py-0.5 bg-pink-500  text-white flex items-center gap-0.5 rounded-full">{ratings.toFixed(1)} <StarIcon sx={{ fontSize: "14px" }} /></span>
                ...
                <div className="flex items-center gap-1.5 text-md font-medium">
                    ...
                    <span className="text-xs text-primary-lightGreen">{getDiscount(price, cuttedPrice)}%&nbsp;off</span>
                </div>
                {/* <!-- price container --> */}
            </div>
            {/* <!-- product description --> */}
